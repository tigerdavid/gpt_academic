...
"""<small>输入问题 → 点提交 | 上传文件 → 点击对应插件 | <a href="https://github.com/binary-husky/gpt_academic">GitHub</a></small>"""
...
def main():
    ...
    DARK_MODE, INIT_SYS_PROMPT, ADD_WAIFU, TTS_TYPE, SIMPLE_UI = get_conf('DARK_MODE', 'INIT_SYS_PROMPT', 'ADD_WAIFU', 'TTS_TYPE', 'SIMPLE_UI')
    ...
    with gr.Blocks(title="GPT 学术优化", theme=set_theme, analytics_enabled=False, css=advanced_css) as app_block:
        ...
        file_upload_2.upload(on_file_uploaded, [file_upload, chatbot, txt, txt2, checkboxes, cookies], [chatbot, txt, txt2, cookies])  # file_upload_2 hidden, route to main upload.then(None, None, None, _js=r"()=>{toast_push('上传完毕 ...'); cancel_loading_status();}")
        # 函数插件-固定按钮区
        for k in plugins:
